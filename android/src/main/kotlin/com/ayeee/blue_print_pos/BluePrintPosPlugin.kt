                            ...
                            webView.evaluateJavascript("document.body.offsetWidth") { offsetWidth ->
                                webView.evaluateJavascript("document.body.offsetHeight") { offsetHeight ->
                                    ...
                                    if (offsetWidth != null && offsetWidth.isNotEmpty() && offsetHeight != null && offsetHeight.isNotEmpty()) {
                                        var offHeight = offsetHeight.toDouble()
                                        if (offHeight > 700 && offHeight < 800){
                                            offHeight = offHeight - (offHeight * 0.2)
                                        }
                                        if (offHeight >= 800 && offHeight < 900){
                                            offHeight = offHeight - (offHeight * 0.25)
                                        }
                                        if (offHeight >= 900 && offHeight < 950){
                                            offHeight = offHeight - (offHeight * 0.28)
                                        }
                                        if (offHeight >= 950 && offHeight < 2000){
                                            offHeight = offHeight - (offHeight * 0.3)
                                        }
                                        Logger.log("\n offHeight : $offHeight")
                                        val data = webView.toBitmap(
                                            offsetWidth.toDouble(), offHeight
                                        )
                                        if (data != null) {
